...
const EditUserPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update user data (only fields supported by UpdateUserRequest)
      const updateData = {
        name: formData.name,
      ...
      if (response.success) {
        // Handle password update if it's been changed (not the masked value)
        if (formData.password.trim() && formData.password !== '********') {
          console.log('Updating password...');
          try {
            await apiService.updateUserPassword(userId!, formData.password);
        ...
        // Handle enable/disable status change if needed
        const currentUser = response.user;
        if (currentUser && currentUser.enabled !== formData.enabled) {
        ...
        // Redirect to users list immediately after successful update
  ...
  const handleResetPassword = () => {
    // Clear the password field to prompt for new password
